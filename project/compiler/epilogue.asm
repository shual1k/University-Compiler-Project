...
L_error_fvar_undefined:
        ...
        lea rdx, [rax + 1 + 8]          ; actual characters
...
L_code_ptr_bin_apply:
        mov rsi, qword[rsp + 8 * 2]     ;;; place rsi on arg-count
        cmp rsi, 2
        jne L_error_arg_count_2
        mov rsi, qword[rsp + 8 * 4]     ;;; place rsi on s (the 'top' of the stack)
        mov rbx, rsi                    ;;; rbx = s
        mov rcx, 0                      ;;; use rcx to get s length
.L:
        cmp rsi, sob_nil                ;;; if remaining s is empty 'list'
        je .L_out
        assert_pair(rsi)                ;;; check the type is correct
        mov rsi, SOB_PAIR_CDR(rsi)
        inc rcx
        jmp .L
.L_out:
        mov r9, qword[rsp + 8 * 0]      ;;; r9 = ret
        mov r8, qword[rsp + 8 * 3]      ;;; r8 = f
        assert_closure(r8)              ;;; make sure f is function
        lea rdi, [8*(rcx-2)]            ;;; rdi = how much we need to move the stack down
        sub rsp, rdi                    ;;; rsp points at where the new ret addr should be
        mov rdi, rsp                    ;;; use rdi as pointer to fix stack                   
        mov qword[rdi], r9              ;;; place ret addr in new position
        add rdi, 8*1
        mov rax, SOB_CLOSURE_ENV(r8)    ;;; load f closure into rax
        mov qword[rdi], rax             ;;; place f env addr
        add rdi, 8*1
        mov qword[rdi], rcx             ;;; place new arg-count
        add rdi, 8*1
.L1:
        cmp rcx, 0
        je .L1_out
        ...
        jmp .L1
.L1_out:
        jmp SOB_CLOSURE_CODE(r8)        ;;; go to the function now that the stack is prepared for it
